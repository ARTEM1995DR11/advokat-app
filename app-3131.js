...
function mergeState(d){
  ...
  out.tasks.forEach(function(t){
    if(t && (t.kind==='meeting' || t.kind==='call' || t.kind==='doc')) t.kind='task';
  });
...
function todayTaskRow(t){
  var m=todayMatter(t);
  var context=m?(m.title+(m.number?' · '+m.number:'')):'Без дела';
  return '<div class="today-row task-row" data-act="task" data-id="'+t.id+'">'+
    ...
    '<small class="today-context">'+esc(context)+'</small>'+
...
function taskProjectRow(t){
  ...
  var context=m?([m.number,m.title].filter(Boolean).join(' · ')):'Без дела';
  ...
  return '<div class="pt-item" data-id="'+t.id+'">'+
    ...
    '<div class="pt-row'+(t.done?' done':'')+'" data-id="'+t.id+'">'+
      ...
      '<div class="pt-main">'+
        ...
        (m?'<button class="pt-link" data-act="task-matter" data-id="'+m.id+'">'+esc(context)+'</button>':'<span class="pt-context">'+esc(context)+'</span>')+
...
function renderMore(){
  ...
  '</div>'+
  '<div class="footnote">Ежедневник адвоката · iPhone Offline 3.1.29<br>'+esc(offlineStatusText())+'<br>Рабочая база хранится локально в зашифрованном виде.</div>';
...
if('serviceWorker' in navigator){
  window.addEventListener('load',function(){
    ...
    navigator.serviceWorker.register('./sw-3131.js', {updateViaCache:'none'}).then(function(reg){
