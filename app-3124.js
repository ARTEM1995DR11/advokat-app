...
function taskActionButtons(t){
  ...
  if(!t.done){
    ...
    acts.push('<button class="pt-act" data-act="task-move-date" data-id="'+t.id+'">Дата</button>');
  } else {
...
function taskProjectRow(t){
  ...
  return '<div class="pt-item" data-id="'+t.id+'">'+
    '<div class="pt-swipe-bg"><span class="pt-swipe-done">'+ico('check','s')+'Готово</span><span class="pt-swipe-more">Действия'+ico('more','s')+'</span></div>'+ 
...
/* =====================================================================
   Swipe по задачам — iPhone touch events (3.1.24)
   Вправо: выполнить / вернуть в работу. Влево: быстрые действия.
   Левые 44px зарезервированы для навигационного Back-swipe.
   ===================================================================== */
...
document.addEventListener('touchmove',function(e){
  ...
  if(!TASK_TOUCH.horizontal){
    if(Math.abs(TASK_TOUCH.dy)>11 && Math.abs(TASK_TOUCH.dy)>Math.abs(TASK_TOUCH.dx)*1.15){taskTouchReset(true);return;}
    if(Math.abs(TASK_TOUCH.dx)>8 && Math.abs(TASK_TOUCH.dx)>Math.abs(TASK_TOUCH.dy)*1.08) TASK_TOUCH.horizontal=true;
  }
  if(!TASK_TOUCH.horizontal) return;
  if(e.cancelable)e.preventDefault();
  var x=Math.max(-92,Math.min(92,TASK_TOUCH.dx));
  TASK_TOUCH.row.style.transform='translate3d('+x+'px,0,0)';
  TASK_TOUCH.row.classList.toggle('swipe-right',x>12);
  TASK_TOUCH.row.classList.toggle('swipe-left',x<-12);
},{passive:false,capture:true});
document.addEventListener('touchend',function(e){
  ...
  var row=TASK_TOUCH.row;
  taskTouchReset(true);
  if(!h || !id || Math.abs(dx)<54 || Math.abs(dx)<Math.abs(dy)*1.12) return;
  SWIPE_CLICK_BLOCK_UNTIL=Date.now()+480;
  if(dx>0){
    closeTaskActionRows();
    setTimeout(function(){toggleTaskDone(id);},80);
  }else{
    setTimeout(function(){toggleTaskActionRow(id);},80);
  }
},{passive:true,capture:true});
document.addEventListener('touchcancel',function(){if(TASK_TOUCH.on)taskTouchReset(true);},{passive:true,capture:true});
...
if('serviceWorker' in navigator){
  window.addEventListener('load',function(){
    ...
    navigator.serviceWorker.register('./sw-3124.js', {updateViaCache:'none'}).then(function(reg){
