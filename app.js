...
function headerSearch(action,active,label){
  ...
  return '<button class="'+cls+'" style="'+style+'" data-act="'+esc(action)+'" title="'+esc(label)+'" aria-label="'+esc(label)+'" type="button"><img class="premium-action-art" src="header-search-premium.png?v=5111" alt=""></button>';
}
...
function renderPremiumListPicker(){
  ...
  modal.innerHTML='<div class="premium-list-grab"></div>'+ 
    ...
    '<div class="premium-list-sign"><i></i><span><img class="premium-list-sign-logo" src="scale-gold.png?v=5111" alt="Весы правосудия"></span><i></i></div>';
}
...
function drawEditor(preserveScroll){
  ...
  openSheet(
  '<div class="task-editor-brand"><img src="scale-gold.png?v=5111" alt="Весы правосудия"><div><b>Ежедневник адвоката</b><small>Больше, чем календарь</small></div></div>'+
...
function quickItem(i,t,act,tone){
  var art={
    'qa-hearing':'quick-icon-hearing.png?v=5111',
    'qa-meeting':'quick-icon-meeting.png?v=5111',
    'qa-deadline':'quick-icon-deadline.png?v=5111',
    'qa-task':'quick-icon-task.png?v=5111',
    'qa-journal':'quick-icon-journal.png?v=5111'
  }[act]||'';
...
function premiumActionIcon(type,cls){
  ...
  var src=type==='bell'?'header-bell-premium.png?v=5111':type==='search'?'header-search-premium.png?v=5111':type==='plus'?'fab-plus-square-premium.png?v=5111':'';
...
function sheetNotify(){
  ...
  var isExactOn = !!(S.settings.notify && ('Notification' in window) && Notification.permission==='granted');
  var overlay = '';
  if(!isExactOn){
    var statusIco=meta.tone==='ok'?'check':(meta.tone==='off'?'bell':'info');
    overlay = '<div class="rem111-statecopy">'+
      '<h3>'+meta.title+'</h3><p>'+meta.text+'</p>'+
      '<span class="rem111-status '+meta.tone+'"><span>'+ico(statusIco,'s')+'</span>'+meta.chip+'</span>'+
    '</div>'+
    '<div class="rem111-off-toggle" data-act="notify" role="button" tabindex="0">'+
      '<span>'+ico('bell','s')+'</span><b>Включить напоминания</b><span>'+ico('chev','s')+'</span>'+
    '</div>';
  }else{
    overlay = '<div class="rem111-toggle-hit" data-act="notify" role="button" tabindex="0" aria-label="Выключить напоминания"></div>';
  }
  openSheet('<section class="rem111">'+
    '<img class="rem111-art" src="reminder-approved-screen-sheet.png?v=5111" alt="Напоминания">'+
    overlay+
  '</section>');
  $('#sheet').classList.add('reminders-approved-v111-sheet');
}
