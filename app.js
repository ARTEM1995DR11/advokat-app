...
var DEF = {
  ...
  settings: {
    name:'', dayRate:0, cur:'₽', notify:false,
...
function mergeState(d){
  ...
  // 5.0.59: приложение окончательно переведено на единую светлую тему.
  delete out.settings.theme;
  delete out.settings.themeAutoV3114;
...
function iconBody(n){
  var g=document.getElementById('i-'+n);
  return g?g.innerHTML:'';
}
function ico(n,c){
  /* 5.0.249: inline SVG geometry instead of <use href="#...">.
...
function materializeUseIcons(){
  /* Also convert icons that are already present in index.html before the first render. */
  try{
    document.querySelectorAll('svg use[href^="#i-"]').forEach(function(use){
      var href=use.getAttribute('href')||'';
      var id=href.slice(1),source=document.getElementById(id);
      if(!source)return;
      var holder=document.createElementNS('http://www.w3.org/2000/svg','g');
      holder.innerHTML=source.innerHTML;
      var parent=use.parentNode;
      while(holder.firstChild) parent.insertBefore(holder.firstChild,use);
...
function premiumDateControl(id,value,emptyLabel,compact){
  var v=value||'', label=v?(compact?fmtD(v,false):fmtD(v,true)):(emptyLabel||'Выберите дату');
  return '<input type="hidden" id="'+esc(id)+'" value="'+esc(v)+'">'+
    '<button type="button" class="premium-date-field'+(v?'':' empty')+'" data-act="date-open" data-target="'+esc(id)+'" data-date-for="'+esc(id)+'"'+(compact?' data-date-compact="1"':'')+'>'+
...
function applyPremiumDatePicker(){
  ...
  var value=dp.selected||'';
  var inp=dp.target?$('#'+dp.target):null;
  if(inp)inp.value=value;

  // Все кнопки выбора даты, включая специальную форму «Заседание»,
  // синхронизируются одинаково. Раньше hearing-кнопка не обновлялась,
  // потому что код искал только .premium-date-field-copy.
  var btn=dp.target?document.querySelector('[data-date-for="'+dp.target+'"]'):null;
  if(btn){
    btn.classList.toggle('empty',!value);
    var b=btn.querySelector('.premium-date-field-copy b, .hearing-premium-picker-copy b');
    if(!b)b=btn.querySelector('b');
    if(b){
      var compact=btn.dataset.dateCompact==='1'||btn.classList.contains('hearing-premium-picker-date');
      b.textContent=value?fmtD(value,!compact):'Выберите дату';
    }
  }

  // Фиксируем дату и в текущих моделях до закрытия календаря.
  if(ED){
    if(dp.target==='e-due')ED.due=value;
    if(dp.target==='e-source')ED.sourceDate=value;
  }
  if(dp.target==='hr-next-date'&&HR)HR.nextDate=value;
...
function listPickerMeta(target){
  ...
  var map={
    ...
    'e-court-choice':{title:'Выбор суда',sub:'Выберите суд или участок',search:'Поиск по судам…',icon:'court'},
...
function premiumListItemExtra(target,value,label){
  ...
  if((target==='e-mid'||target==='pt-mid')&&!value) return 'Запись без привязки к конкретному делу';
  ...
  if(target==='m-execution-issue'&&value){
    var exi=criminalExecutionIssue(value);
    if(exi){
      var jshort=exi.jurisdiction==='sentence'
        ? 'суд приговора; при исполнении вне его юрисдикции — суд того же уровня по месту исполнения'
        : exi.jurisdictionLabel;
      return exi.article+' · '+jshort;
    }
  }
...
function premiumListMatterMeta(target,value){
  ...
  if((target==='e-court-choice'||target==='m-court-choice')&&value){
    var orgChoice=target==='m-court-choice'?matterInvestigationOrgByValue(value):null;
    if(orgChoice)return {type:'org-'+orgChoice.kind,color:orgChoice.c||'#7A8FA6',icon:'brief'};
    var courtChoice=commonCourtByValue(value);
    if(courtChoice)return {type:courtChoice.main?'court-city':'court-world',color:'#C9962B',icon:'court'};
  }
...
function premiumMatterPickerBasis(value){
  ...
  // Для старых сохранённых дел без basis сохраняем ожидаемое поведение:
  // считаем их делами по соглашению, поскольку именно это основание является
  // базовым значением при создании нового дела.
  var key=m.basis==='assigned'?'assigned':'agreement';
  var meta=MATTER_BASIS[key]||MATTER_BASIS.agreement;
  return {key:key,label:meta.n,short:meta.short,color:meta.c};
}
...
function openPremiumListPicker(target,inputId){
  ...
  var meta=listPickerMeta(target),isMatterPicker=premiumListIsMatterPicker(target),items=Array.prototype.slice.call(sel.options).map(function(o){
    ...
    var basis=isMatterPicker?premiumMatterPickerBasis(o.value):null;
    return {value:o.value,label:(o.textContent||o.label||o.value||'').trim(),disabled:!!o.disabled,extra:premiumListItemExtra(target,o.value,(o.textContent||'').trim()),matterMeta:mm,basisMeta:basis};
  });
  ...
  var selectedValue=premiumListSelectedValue(sel,inputId||'');
  var selectedBasis=isMatterPicker?premiumMatterPickerBasis(selectedValue):null;
  LIST_PICKER={target:target,inputId:inputId||'',selected:selectedValue,query:'',items:items,meta:meta};
  if(isMatterPicker){
    // Первый экран выбора дела — только группы оснований без раскрытого списка.
    // Дальше адвокат осознанно выбирает «по соглашению» или «по назначению»,
    // и только затем видит конкретные дела внутри выбранной категории.
...
function closePremiumListPicker(){
  var modal=$('#premium-list-modal'),scr=$('#list-scrim');if(modal){modal.classList.remove('open','premium-matter-picker','premium-matter-choice-v1');}if(scr)scr.classList.remove('open');
...
function matterEditorPickerWithoutSearch(target){
  ...
  return ['m-type','m-basis','m-stage','m-role','m-court-choice','m-restraint-choice'].indexOf(target)>=0;
}
function premiumMatterPickerRow(it){
  ...
  return '<button type="button" class="'+rowClass+'"'+rowStyle+' data-act="list-pick" data-v="'+esc(it.value)+'"'+(it.disabled?' disabled':'')+'>'+ 
    ...
    '<span class="premium-list-row-end">'+end+'</span></button>';
}
function premiumMatterPickerGroups(rows){
  var defs=[
    {key:'agreement',label:'По соглашению',color:(MATTER_BASIS.agreement||{}).c||'#2FA08E',icon:'brief'},
    {key:'assigned',label:'По назначению',color:(MATTER_BASIS.assigned||{}).c||'#8A6AD8',icon:'user'}
  ];
  var known={agreement:[],assigned:[]},other=[];
  rows.forEach(function(it){
    var key=it.basisMeta&&it.basisMeta.key;
    if(key&&known[key])known[key].push(it);else other.push(it);
  });
  var searching=!!String((LIST_PICKER&&LIST_PICKER.query)||'').trim();
  var openState=(LIST_PICKER&&LIST_PICKER.groupOpen)||{agreement:true,assigned:false,other:false};
  function groupHtml(g,arr){
    if(!arr.length)return '';
    var open=searching?true:openState[g.key]!==false;
    return '<section class="premium-matter-group premium-matter-group-'+g.key+(open?' open':' collapsed')+'" style="--basis-color:'+esc(g.color)+'">'+
      '<button type="button" class="premium-matter-group-head" data-act="matter-group-toggle" data-v="'+esc(g.key)+'" aria-expanded="'+(open?'true':'false')+'">'+
        '<span class="premium-matter-group-symbol">'+ico(g.icon,'s')+'</span>'+
        '<span class="premium-matter-group-copy"><b>'+esc(g.label)+'</b><small>'+arr.length+' '+plural(arr.length,'дело','дела','дел')+'</small></span>'+
        '<span class="premium-matter-group-arrow">'+ico('chev','s')+'</span>'+
      '</button>'+
      (open?'<div class="premium-matter-group-list">'+arr.map(premiumMatterPickerRow).join('')+'</div>':'')+
    '</section>';
  }
  var html=defs.map(function(g){return groupHtml(g,known[g.key]);}).join('');
  if(other.length) html+=groupHtml({key:'other',label:'Другие дела',color:'#9A8A70',icon:'folder'},other);
  return html;
}
function renderPremiumListPicker(){
  ...
  var rows=LIST_PICKER.items.filter(function(it){
    if(!q)return true;
    var basis=it.basisMeta?((it.basisMeta.label||'')+' '+(it.basisMeta.short||'')):'';
  ...
  modal.classList.toggle('premium-matter-choice-v1',isMatterPicker);
  modal.classList.toggle('premium-matter-single',isMatterPicker&&rows.length===1);
  modal.setAttribute('data-list-target',LIST_PICKER.target||'');
  var hasQuery=!!String(LIST_PICKER.query||'').trim();
  var isCourtPicker=['e-court-choice','m-court-choice'].indexOf(LIST_PICKER.target)>=0;
  ...
  var list='';
  if(rows.length){
    list=isMatterPicker?premiumMatterPickerGroups(rows):rows.map(premiumMatterPickerRow).join('');
  }else{
    list='<div class="premium-list-empty">'+ico('search')+'<b>Ничего не найдено</b><small>Измените поисковый запрос.</small></div>';
  }
  // Для необязательной привязки дела действие «без дела» остаётся отдельной
...
function matterEditorInputShell(id,icon,html){
  return '<div class="matter-editor-input-shell" data-field="'+esc(id)+'"><span class="matter-editor-field-leading">'+ico(icon||matterEditorControlIconName(id)||'doc','s')+'</span>'+html+'</div>';
}
function upgradePremiumSelects(root){
  ...
  Array.prototype.slice.call(root.querySelectorAll('select')).forEach(function(sel){
    ...
    var btn=document.createElement('button');btn.type='button';btn.className='premium-select-field';btn.dataset.act='list-open';btn.dataset.target=sel.id;btn.dataset.premiumSelectFor=sel.id;var matterLead=sel.closest('.matter-editor-sheet')?matterEditorControlIconName(sel.id):'';
    btn.classList.toggle('matter-editor-select-with-icon',!!matterLead);
    btn.innerHTML=(matterLead?'<span class="matter-editor-field-leading">'+ico(matterLead,'s')+'</span>':'')+'<span class="premium-select-field-copy"><b></b></span><span class="premium-select-field-arrow">'+ico('chev','s')+'</span>';
...
function clearPremiumMatterChoice(){
  if(!LIST_PICKER||LIST_PICKER.target!=='e-mid')return;
  var lp=LIST_PICKER,sel=$('#'+lp.target);if(!sel)return;
  sel.value='';
  if(lp.inputId){var inp=$('#'+lp.inputId);if(inp){inp.value='';inp.dispatchEvent(new Event('input',{bubbles:true}));}}
  syncPremiumSelectButton(sel.id);
  sel.dispatchEvent(new Event('change',{bubbles:true}));
  closePremiumListPicker();vib(5);
}
...
function toast(m){
  ...
  var msg=String(m==null?'':m), low=msg.toLowerCase(), iconName='info', extra='';
  if(low.indexOf('напоминания выключены')>=0){iconName='bell';extra=' is-bell-off';}
  else if(low.indexOf('напоминания включены')>=0){iconName='bell';extra=' is-bell-on';}
  else if(low.indexOf('удален')>=0||low.indexOf('удалён')>=0){iconName='trash';extra=' is-danger';}
  else if(low.indexOf('сохран')>=0||low.indexOf('добавлен')>=0||low.indexOf('выполнено')>=0||low.indexOf('восстановлен')>=0){iconName='check';extra=' is-success';}
  else if(low.indexOf('нельзя')>=0||low.indexOf('не удалось')>=0||low.indexOf('укажите')>=0||low.indexOf('просроч')>=0||low.indexOf('разрешение не')>=0){iconName='alert';extra=' is-warning';}
  else if(low.indexOf('офлайн')>=0){iconName='info';extra=' is-offline';}
  t.className='toast premium-toast'+extra+(msg.length>38?' is-long':'');
  t.innerHTML='<span class="premium-toast-icon">'+ico(iconName,'s')+'</span><span class="premium-toast-sep" aria-hidden="true"></span><span class="premium-toast-text">'+esc(msg)+'</span>';
...
var CRIMINAL_EXECUTION_ISSUES = [
  ...
  {id:'replace-evasion',name:'Замена наказания при злостном уклонении от его отбывания',short:'Замена наказания за уклонение / нарушения',article:'ч. 1–2 ст. 396 УПК РФ · п. 2 ст. 397 УПК РФ',jurisdiction:'sentence',jurisdictionLabel:'по общему правилу — суд, постановивший приговор; если приговор исполняется вне его юрисдикции — суд того же уровня по месту исполнения (ч. 1–2 ст. 396 УПК РФ)'},
  {id:'forced-to-prison',name:'Замена принудительных работ лишением свободы при уклонении или злостных нарушениях',short:'Принудительные работы → лишение свободы',article:'ч. 1–2 ст. 396 УПК РФ · п. 2.1 ст. 397 УПК РФ · ч. 4 ст. 53.1 УК РФ · ст. 60.15, 60.17 УИК РФ',jurisdiction:'sentence',jurisdictionLabel:'по общему правилу — суд, постановивший приговор; если приговор исполняется вне его юрисдикции — суд того же уровня по месту исполнения (ч. 1–2 ст. 396 УПК РФ)'},
  ...
  {id:'limitation',name:'Освобождение от наказания вследствие истечения сроков давности приговора',short:'Давность исполнения приговора',article:'п. 9 ст. 397 УПК РФ · ст. 83 УК РФ',jurisdiction:'sentence',jurisdictionLabel:'по общему правилу — суд, постановивший приговор; если приговор исполняется вне его юрисдикции — суд того же уровня по месту исполнения (ч. 1–2 ст. 396 УПК РФ)'},
  {id:'multiple-sentences',name:'Исполнение приговора при наличии других неисполненных приговоров',short:'Несколько приговоров',article:'п. 10 ст. 397 УПК РФ · ст. 70 УК РФ',jurisdiction:'sentence',jurisdictionLabel:'по общему правилу — суд, постановивший приговор; если приговор исполняется вне его юрисдикции — суд того же уровня по месту исполнения (ч. 1–2 ст. 396 УПК РФ)'},
  {id:'credit',name:'Зачёт времени содержания под стражей / пребывания в лечебном учреждении',short:'Зачёт срока',article:'п. 11 ст. 397 УПК РФ · ст. 72, 103, 104 УК РФ',jurisdiction:'sentence',jurisdictionLabel:'по общему правилу — суд, постановивший приговор; если приговор исполняется вне его юрисдикции — суд того же уровня по месту исполнения (ч. 1–2 ст. 396 УПК РФ)'},
  {id:'deductions',name:'Снижение удержаний из заработной платы при исправительных работах',short:'Снижение удержаний',article:'п. 14 ст. 397 УПК РФ · ст. 44 УИК РФ',jurisdiction:'sentence',jurisdictionLabel:'по общему правилу — суд, постановивший приговор; если приговор исполняется вне его юрисдикции — суд того же уровня по месту исполнения (ч. 1–2 ст. 396 УПК РФ)'},
  {id:'clarify',name:'Разъяснение сомнений и неясностей при исполнении приговора',short:'Разъяснение приговора',article:'п. 15 ст. 397 УПК РФ',jurisdiction:'sentence',jurisdictionLabel:'по общему правилу — суд, постановивший приговор; если приговор исполняется вне его юрисдикции — суд того же уровня по месту исполнения (ч. 1–2 ст. 396 УПК РФ)'},
  {id:'minor',name:'Освобождение несовершеннолетнего от наказания с применением мер воспитательного воздействия',short:'Несовершеннолетний · освобождение',article:'п. 16 ст. 397 УПК РФ · ст. 92 УК РФ',jurisdiction:'sentence',jurisdictionLabel:'по общему правилу — суд, постановивший приговор; если приговор исполняется вне его юрисдикции — суд того же уровня по месту исполнения (ч. 1–2 ст. 396 УПК РФ)'},
  {id:'defer',name:'Отсрочка исполнения приговора / отсрочка или рассрочка штрафа',short:'Отсрочка исполнения приговора',article:'ст. 398 УПК РФ',jurisdiction:'sentence',jurisdictionLabel:'по общему правилу — суд, постановивший приговор; если приговор исполняется вне его юрисдикции — суд того же уровня по месту исполнения (ч. 1–2 ст. 396 УПК РФ)'},
  ...
  {id:'rehabilitation',name:'Возмещение вреда и восстановление прав реабилитированного',short:'Реабилитация',article:'п. 1 ст. 397 УПК РФ',jurisdiction:'sentence',jurisdictionLabel:'по общему правилу — суд, постановивший приговор; если приговор исполняется вне его юрисдикции — суд того же уровня по месту исполнения (ч. 1–2 ст. 396 УПК РФ)'},
  {id:'property',name:'Меры по обеспечению сохранности имущества или жилого помещения',short:'Сохранность имущества',article:'п. 22 ст. 397 УПК РФ · ст. 313.1 УПК РФ',jurisdiction:'sentence',jurisdictionLabel:'по общему правилу — суд, постановивший приговор; если приговор исполняется вне его юрисдикции — суд того же уровня по месту исполнения (ч. 1–2 ст. 396 УПК РФ)'},
  {id:'foreign-transfer',name:'Передача осуждённого / исполнения наказания иностранному государству',short:'Передача исполнения иностранному государству',article:'п. 20, 20.1 ст. 397 УПК РФ',jurisdiction:'sentence',jurisdictionLabel:'по общему правилу — суд, постановивший приговор; если приговор исполняется вне его юрисдикции — суд того же уровня по месту исполнения (ч. 1–2 ст. 396 УПК РФ)'},
...
function openSheet(html){
  var s = $('#sheet');
  s.classList.remove('full','quick-sheet','task-editor-sheet','hearing-premium-sheet','entry-premium-sheet','qe190-sheet','qe192-hearing-sheet','editor-inline-foot','hearing-result-sheet','filter-premium-sheet','task-filter-premium','matter-filter-premium','matter-editor-sheet','task-actions-premium','matter-actions-premium','sheet-premium-form','sheet-premium-search','notify-premium-sheet','reminders-approved-v107-sheet','reminders-approved-v108-sheet','reminders-approved-v114-sheet','reminders-approved-v116-sheet','reminders-approved-v117-sheet','global-search-premium-v166-sheet');
...
function hearingCaption(t,m){
  /* 5.0.377: главным заголовком заседания является название/суть дела,
     а номер дела показывается отдельной строкой ниже. */
  if(m){
    var title=String(m.title||'').replace(/\s+/g,' ').trim();
    if(title) return matterAutoTitlePiece(title,86);
    var auto=matterAutoTitle({
      type:m.type||'other', client:m.client||'', article:m.article||'', notes:m.notes||'',
      stage:m.stage||'', executionIssue:m.executionIssue||''
    });
    if(auto) return matterAutoTitlePiece(auto,86);
    if(m.client) return matterAutoClientLabel(m.client);
  }
  return t.hearingTopic||t.hearingClient||t.hearingNumber||'Судебное заседание';
}
function hearingCaseStageLine(t,m){
  var parts=[];
  if(m&&m.number) parts.push('№ '+m.number);
  else if(!m&&t.hearingNumber) parts.push('№ '+t.hearingNumber);
...
function hearingCaseBasisLine(t,m){
  var caseLine=hearingCaseStageLine(t,m);
  if(!caseLine) return '';
  return '<small class="hearing-unified-case hearing-case-basis-line">'+
    '<span class="hearing-case-text">'+esc(caseLine)+'</span>'+ 
  '</small>';
}
...
function hearingUnifiedCourtLine(t,m){
  var place=hearingPlace(t,m)||'Суд не указан';
  return '<small class="hearing-unified-court">'+esc(place)+'</small>';
}
...
function todayNext7Row(t){
  ...
  return '<button class="today-row upcoming-row next7-row '+cls+'" data-act="task" data-id="'+t.id+'">'+
    ...
    '<span class="today-row-main">'+
      ...
      (t.kind==='hearing'?hearingTitleBasisRow(t,m):'<b>'+esc(title)+'</b>')+
      (t.kind!=='hearing'&&context?'<small class="today-kind-context">'+esc(context)+'</small>':'')+
      line2+line3+
    '</span>'+ico('chev','s')+'</button>';
}
...
function renderToday(){
  ...
  /* Финальный приоритет экрана «Сегодня»: сроки → заседания → встречи → задачи → ближайшее. */
...
function premiumDeleteCopy(t){
  var m={
    task:{title:'Удалить задачу?',text:'Это действие нельзя отменить.'},
    meeting:{title:'Удалить встречу?',text:'Это действие нельзя отменить.'},
    deadline:{title:'Удалить процессуальный срок?',text:'Это действие нельзя отменить.'},
    hearing:{title:'Удалить заседание?',text:'Это действие нельзя отменить.'}
  };
  return m[(t&&t.kind)||'task']||m.task;
}
function closePremiumDeleteConfirm(){
  var root=document.getElementById('premium-delete-confirm-root');
  if(!root)return;
  root.classList.remove('open');
  setTimeout(function(){if(root.parentNode)root.parentNode.removeChild(root);},180);
}
function openPremiumDeleteConfirm(id){
  var t=S.tasks.filter(function(x){return x.id===id;})[0];
  if(!t)return;
  ...
  root.innerHTML='<button type="button" class="premium-delete-confirm-backdrop" data-act="premium-delete-cancel" aria-label="Отмена"></button>'+ 
    '<section class="premium-delete-confirm" role="dialog" aria-modal="true" aria-labelledby="premium-delete-confirm-title">'+
      ...
      '<h2 id="premium-delete-confirm-title">'+esc(copy.title)+'</h2>'+ 
      '<p>'+esc(copy.text)+'</p>'+ 
      '<div class="premium-delete-confirm-actions">'+
        ...
        '<button type="button" class="premium-delete-accept" data-act="premium-delete-confirm" data-id="'+esc(id)+'">'+ico('trash','s')+'<span>Удалить</span></button>'+ 
      '</div>'+ 
    '</section>';
...
function deleteTaskById(id){ openPremiumDeleteConfirm(id); }
...
function drawHearingResultSheet(){
  ...
  var hrJudge=(hearingJudgeName(t,m)||t.hearingJudge||'Судья не указан').trim();
  if(hrJudge && !/^судья/i.test(hrJudge) && hrJudge!=='Судья не указан') hrJudge='Судья '+hrJudge;
...
function taskProjectMatch(t,chip){
  var active=!t.done && !meetingOccurred(t), urgent=urgentTaskNeedsToday(t);
  if(chip==='late') return active && (t.kind==='task'||t.kind==='deadline') && t.due && dd(t.due)<0;
  if(chip==='today') return active && ((t.due && dd(t.due)===0) || urgent);
  if(chip==='week') return active && !urgent && t.due && dd(t.due)>0 && dd(t.due)<=7;
  if(chip==='later') return active && !urgent && t.due && dd(t.due)>7;
  if(chip==='nodue') return active && !urgent && !t.due;
...
function taskProjectCounts(){
  ...
  return {
    ...
    today:active.filter(function(t){return (t.due&&dd(t.due)===0)||urgentTaskNeedsToday(t);}).length,
    week:active.filter(function(t){return !urgentTaskNeedsToday(t)&&t.due&&dd(t.due)>0&&dd(t.due)<=7;}).length,
    later:active.filter(function(t){return !urgentTaskNeedsToday(t)&&t.due&&dd(t.due)>7;}).length,
    nodue:active.filter(function(t){return !urgentTaskNeedsToday(t)&&!t.due;}).length,
...
function taskProjectGroups(list){
  ...
  list.forEach(function(t){
    ...
    if(urgentTaskNeedsToday(t)){out[4].items.push(t);return;}
...
<defs>
  <linearGradient id="@rearTab" x1="0" y1="0" x2=".95" y2="1"><stop stop-color="var(--folder-deep)"/><stop offset=".42" stop-color="var(--folder)"/><stop offset="1" stop-color="var(--folder-deep)"/></linearGradient>
  <linearGradient id="@frontTab" x1=".08" y1="0" x2=".9" y2="1"><stop stop-color="#fff" stop-opacity=".82"/><stop offset=".13" stop-color="var(--folder-light)"/><stop offset=".72" stop-color="var(--folder-light)"/><stop offset="1" stop-color="var(--folder)"/></linearGradient>
  <linearGradient id="@paper" x1="0" y1="0" x2="0" y2="1"><stop stop-color="#fffefa"/><stop offset=".56" stop-color="#fffaf2"/><stop offset="1" stop-color="#ecdbbc"/></linearGradient>
  <linearGradient id="@paperEdge" x1="0" y1="0" x2="1" y2="0"><stop stop-color="#fdf7e9"/><stop offset=".65" stop-color="#e9d3ad"/><stop offset="1" stop-color="#b98235"/></linearGradient>
  <linearGradient id="@ivory" x1=".02" y1="0" x2=".96" y2="1"><stop stop-color="#fffefb"/><stop offset=".34" stop-color="#fffaf2"/><stop offset=".72" stop-color="#fbf3e5"/><stop offset="1" stop-color="#eedfc3"/></linearGradient>
  <radialGradient id="@faceLight" cx=".2" cy=".02" r="1"><stop stop-color="#fff" stop-opacity=".9"/><stop offset=".44" stop-color="#fff" stop-opacity=".18"/><stop offset="1" stop-color="#fff" stop-opacity="0"/></radialGradient>
  <linearGradient id="@faceDepth" x1="0" y1="0" x2="1" y2="1"><stop offset=".56" stop-color="#c9923f" stop-opacity="0"/><stop offset=".84" stop-color="#a76c1e" stop-opacity=".04"/><stop offset="1" stop-color="#805015" stop-opacity=".11"/></linearGradient>
  <linearGradient id="@goldEdge" x1="0" y1="0" x2=".9" y2="1"><stop stop-color="#f8e4aa"/><stop offset=".18" stop-color="#fff3cf"/><stop offset=".43" stop-color="#bd8527"/><stop offset=".7" stop-color="#efd58c"/><stop offset="1" stop-color="#9b6112"/></linearGradient>
  <linearGradient id="@metal" x1="0" y1="0" x2="1" y2="1"><stop stop-color="#684006"/><stop offset=".18" stop-color="#d9a340"/><stop offset=".4" stop-color="#fff1ad"/><stop offset=".6" stop-color="#c8881b"/><stop offset=".82" stop-color="#f2c765"/><stop offset="1" stop-color="#724507"/></linearGradient>
  <linearGradient id="@pin" x1="0" y1="0" x2="1" y2="0"><stop stop-color="#744306"/><stop offset=".35" stop-color="#d9a13b"/><stop offset=".53" stop-color="#fff0a8"/><stop offset=".72" stop-color="#e8b44e"/><stop offset="1" stop-color="#7e4b07"/></linearGradient>
  <filter id="@tabShadow" x="-18%" y="-35%" width="155%" height="205%"><feDropShadow dx="0" dy="2.7" stdDeviation="2.5" flood-color="#593b18" flood-opacity=".26"/></filter>
  <filter id="@paperShadow" x="-20%" y="-40%" width="150%" height="205%"><feDropShadow dx=".8" dy="1.8" stdDeviation="1.6" flood-color="#6f512c" flood-opacity=".2"/></filter>
  <filter id="@bodyShadow" x="-12%" y="-12%" width="130%" height="138%"><feDropShadow dx="2.2" dy="5.2" stdDeviation="4.8" flood-color="#5e401c" flood-opacity=".22"/></filter>
  <filter id="@metalShadow" x="-75%" y="-60%" width="270%" height="285%"><feDropShadow dx="1.1" dy="1.8" stdDeviation="1.35" flood-color="#5c3505" flood-opacity=".45"/></filter>
  <path id="@front" d="M25 112C16 112 11 119 10 131C9 153 10 181 10 211L10 402C10 426 16 443 31 453C44 462 64 464 92 462C171 459 272 459 352 462C382 464 405 460 419 449C430 440 433 423 432 401L432 149C432 130 427 119 416 114C405 109 389 110 371 111C335 113 307 113 277 112C213 109 154 110 101 111C67 111 42 111 25 112Z"/>
  <clipPath id="@frontClip"><use href="#@front"/></clipPath>
</defs>

<!-- compact coloured rear shell: visible as real folder material, not a side border -->
<path d="M13 109V67Q13 52 25 45Q28 33 40 28L49 18Q57 10 72 10H202Q220 10 231 25L250 55Q258 69 271 80Q283 90 302 95L326 99V109H13Z" fill="url(#@rearTab)" stroke="var(--folder-deep)" stroke-width="1.5" filter="url(#@tabShadow)"/>
<path d="M22 97V67Q22 57 31 51Q36 39 47 35L56 25Q62 19 74 19H198Q213 19 221 31L239 59Q247 72 260 82" fill="none" stroke="#fff" stroke-opacity=".24" stroke-width="2.2" stroke-linecap="round"/>

<!-- paper fan: thicker, irregular lengths/heights, slight offsets like real documents -->
<g fill="url(#@paper)" stroke="url(#@paperEdge)" stroke-width="1.15" filter="url(#@paperShadow)">
  <path d="M209 41H324Q335 41 336 51L337 109H209Z"/>
  <path d="M219 49H349Q360 49 361 59L362 111H219Z"/>
  <path d="M232 58H374Q385 58 386 68L387 113H232Z"/>
  <path d="M247 69H401Q411 69 412 79L412 115H247Z"/>
</g>
<g fill="none" stroke="#fffdf7" stroke-width="1.45" stroke-linecap="round" opacity=".95">
  <path d="M216 46H323"/><path d="M226 54H348"/><path d="M239 63H373"/><path d="M254 74H399"/>
</g>
<g fill="none" stroke="#b98942" stroke-opacity=".22" stroke-width=".7">
  <path d="M337 52L337 107"/><path d="M362 60L362 109"/><path d="M387 69L387 111"/><path d="M412 80L412 113"/>
</g>

<!-- broad front tab: taller, softer, and visually dominant like the approved mockup -->
<path d="M28 105L39 62Q42 49 53 42L63 30Q71 20 86 20H196Q214 20 224 35L243 65Q251 79 264 88Q278 98 299 100L315 101V108H28Z" fill="url(#@frontTab)" stroke="#fff8e8" stroke-opacity=".84" stroke-width="1.5" filter="url(#@tabShadow)"/>
<path d="M38 96L47 64Q50 54 59 48L68 37Q74 30 86 30H192Q205 30 213 41L231 69Q238 80 250 88" fill="none" stroke="#fff" stroke-opacity=".56" stroke-width="1.7" stroke-linecap="round"/>
<path d="M224 36L243 66Q251 79 263 88Q276 97 296 100" fill="none" stroke="var(--folder-deep)" stroke-opacity=".18" stroke-width="2.2" stroke-linecap="round"/>

<!-- front pocket: noticeably different physical silhouette from the old rounded rectangle -->
<use href="#@front" transform="translate(2.1 3.1)" fill="#dfca9f" fill-opacity=".72" stroke="#9f6719" stroke-opacity=".16" stroke-width="1.25"/>
<use href="#@front" fill="url(#@ivory)" stroke="url(#@goldEdge)" stroke-width="1.8" filter="url(#@bodyShadow)"/>
<g clip-path="url(#@frontClip)">
  <rect x="5" y="108" width="432" height="378" fill="url(#@faceLight)"/>
  <rect x="5" y="108" width="432" height="378" fill="url(#@faceDepth)"/>
  <!-- warm edge depth: gives the face a thick leather/paper-board feel -->
  <path d="M18 138C18 122 27 117 42 116C101 113 163 113 223 114C286 115 347 114 404 114C419 114 425 122 425 139L425 394C425 416 421 432 411 441C400 451 384 454 363 452C281 449 171 449 89 452C67 454 50 451 39 442C27 433 22 419 22 399L22 158C22 150 21 143 18 138" fill="none" stroke="#fffdf7" stroke-opacity=".78" stroke-width="1.4" stroke-linecap="round"/>
  <path d="M424 151L424 397C424 419 419 434 407 444" fill="none" stroke="#b37c2c" stroke-opacity=".10" stroke-width="2.2" stroke-linecap="round"/>
  <path d="M39 452C139 449 257 449 362 452" fill="none" stroke="#b37c2c" stroke-opacity=".12" stroke-width="2" stroke-linecap="round"/>
</g>

<!-- top overlap seam: front pocket visibly sits over tab/papers -->
<path d="M24 112C90 109 157 110 225 111C287 112 345 111 415 112" fill="none" stroke="#fff6df" stroke-width="3.1" stroke-linecap="round"/>
<path d="M24 114C93 111 160 112 226 113C291 114 348 113 415 114" fill="none" stroke="#9d691e" stroke-opacity=".25" stroke-width=".9" stroke-linecap="round"/>

<!-- larger metal eyelet with a longer pin, matching the mockup's jewellery-like fitting -->
<ellipse cx="29" cy="111" rx="10.3" ry="4.1" fill="#684009" opacity=".2"/>
<g filter="url(#@metalShadow)">
  <circle cx="29" cy="103" r="12.2" fill="#fff8dc" stroke="url(#@metal)" stroke-width="6.2"/>
  <circle cx="29" cy="103" r="7.5" fill="#fff9e9" stroke="#865616" stroke-width=".8"/>
  <path d="M20.7 97.8A11.8 11.8 0 0 1 37.2 97" fill="none" stroke="#fff4b8" stroke-width="1.9" stroke-linecap="round"/>
  <rect x="25.2" y="112" width="7.6" height="31" rx="3.8" fill="url(#@pin)" stroke="#8f5a0e" stroke-width=".7"/>
  <path d="M28.2 116V137" stroke="#fff0ad" stroke-width="1.35" stroke-linecap="round"/>
</g>
...
function renderMatters(){
  ...
  var html='<div class="matters-project matters-approved-v3 case-page">'+
    ...
    '<div class="case-grid-offset'+(searchOpen?' is-search-open':'')+'">'+
      (searchOpen
