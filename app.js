...
function renderPremiumListPicker(){
  ...
  modal.innerHTML='<div class="premium-list-grab"></div>'+ 
    ...
    '<div class="premium-list-sign"><i></i><span><img class="premium-list-sign-logo" src="scale-gold.png?v=5152" alt="Весы правосудия"></span><i></i></div>';
}
...
function drawEditor(preserveScroll){
  ...
  openSheet(
  '<div class="task-editor-brand"><img src="scale-gold.png?v=5152" alt="Весы правосудия"><div><b>Ежедневник адвоката</b><small>Больше, чем календарь</small></div></div>'+
...
function quickItem(i,t,act,tone){
  ...
  var art={
    'qa-hearing':'quick-card-hearing-v5100.png?v=5152',
    'qa-meeting':'quick-card-meeting-v5100.png?v=5152',
    'qa-deadline':'quick-card-deadline-v5100.png?v=5152',
    'qa-task':'quick-card-task-v5100.png?v=5152',
    'qa-journal':'quick-card-journal-v5100.png?v=5152'
  }[act]||'';
  if(art){
...
function premiumActionIcon(type,cls){
  ...
  var src=type==='bell'?'header-bell-premium.png?v=5152':type==='search'?'header-search-premium.png?v=5152':type==='plus'?'fab-plus-square-premium.png?v=5152':'';
...
function sheetGlobalSearch(){
  ...
  openSheet('<section class="gs135">'+
    '<header class="gs135-head">'+
      '<div class="gs135-head-copy"><h2>Глобальный поиск</h2><p><span>Доверители, номера дел, суды, статьи,</span><span>задачи и журнал.</span></p></div>'+
      '<div class="gs135-head-decor" aria-hidden="true">'+
        '<svg class="gs135-ribbons" viewBox="0 0 190 190" preserveAspectRatio="none"><path d="M20 -10 C48 25 50 57 84 87 C116 115 148 124 184 166"/><path d="M44 -10 C68 22 71 52 100 78 C130 102 157 113 194 148"/></svg>'+
        '<img class="gs135-medallion" src="search-medallion.png?v=5152" alt="">'+
      '</div>'+
    '</header>'+
    '<label class="gs135-search" for="gq">'+
      '<span class="gs135-search-icon">'+ico('search','s')+'</span>'+
      '<input id="gq" placeholder="Введите запрос..." autocomplete="off" autocapitalize="off" spellcheck="false">'+
      '<span class="gs135-search-sep"></span>'+
      '<button class="gs135-mic" type="button" data-act="global-search-mic" aria-label="Голосовой поиск">'+searchPremiumMicIcon()+'</button>'+
    '</label>'+
    '<div class="gs135-info" id="gsearch-tip">'+
      '<span class="gs135-info-icon"><b>i</b></span>'+
      '<div class="gs135-info-copy">Ищите по фамилии, номеру дела, суду,<br>статье, задаче или части заметки.</div>'+
      '<button type="button" class="gs135-close" data-act="global-search-clear-tip" aria-label="Закрыть подсказку">'+ico('xmark','s')+'</button>'+
    '</div>'+
    '<div class="gs135-main">'+
      '<div class="gs135-examples">'+
        '<div class="gs135-example-title"><span></span><b>ПРИМЕРЫ ЗАПРОСОВ</b><span></span></div>'+
        '<div class="gs135-chips">'+
          searchPremiumChip('Ошарин','Ошарин','user')+
          searchPremiumChip('81 УК','81 УК','page')+
          searchPremiumChip('Ивановский суд','Ивановский суд','court')+
          searchPremiumChip('А56-12345/2024','А56-12345/2024','page')+
        ...
        '</div>'+
      '</div>'+
      '<div id="gresults" class="gs135-results search-premium-results"></div>'+
      '<div class="gs135-helper" id="ghelper">'+
        '<div class="gs135-helper-star">'+ico('sparkles','s')+'</div>'+
        '<div class="gs135-helper-copy"><b>Быстрый доступ к важному</b><p>Используйте поиск, чтобы мгновенно найти<br>нужные дела, документы и задачи.</p></div>'+
        '<button type="button" class="gs135-close" data-act="global-search-clear-tip" aria-label="Закрыть подсказку">'+ico('xmark','s')+'</button>'+
      '</div>'+
    '</div>'+
    '<footer class="gs135-footer"><span></span><img src="reminder-footer-scales-exact.png?v=5152" alt=""><span></span><b>ПРАВО И ПОРЯДОК</b></footer>'+
  '</section>');
  $('#sheet').classList.add('global-search-approved-v135-sheet');
  renderGlobalSearch();
}
function renderGlobalSearch(){
  var box=$('#gresults'); if(!box)return;
  var shell=document.querySelector('.gs135');
  var helper=$('#ghelper');
  var examples=document.querySelector('.gs135-examples');
  var q=(GQ||'').trim();
...
function sheetNotify(){
  ...
  openSheet('<section class="rem116">'+
    '<header class="rem116-head">'+
      ...
      '<img class="rem116-head-motif" src="reminder-head-motif-exact.png?v=5152" alt="">'+
    '</header>'+
    ...
    '<footer class="rem116-footer"><span></span><img class="rem116-footer-scale" src="reminder-footer-scales-exact.png?v=5152" alt=""><span></span><b>ДЕЛА В ПОРЯДКЕ</b></footer>'+
  '</section>');
...
if('serviceWorker' in navigator){
  window.addEventListener('load',function(){
    ...
    navigator.serviceWorker.register('./sw.js?v=5152', {updateViaCache:'none'}).then(function(reg){
