...
function renderPremiumListPicker(){
  ...
  modal.innerHTML='<div class="premium-list-grab"></div>'+ 
    ...
    '<div class="premium-list-sign"><i></i><span><img class="premium-list-sign-logo" src="scale-gold.png?v=5107" alt="Весы правосудия"></span><i></i></div>';
}
...
function openSheet(html){
  var s = $('#sheet');
  s.classList.remove('quick-sheet','task-editor-sheet','hearing-result-sheet','filter-premium-sheet','task-filter-premium','matter-filter-premium','matter-editor-sheet','task-actions-premium','matter-actions-premium','sheet-premium-form','sheet-premium-search','notify-premium-sheet','reminders-approved-v107-sheet');
...
function drawEditor(preserveScroll){
  ...
  openSheet(
  '<div class="task-editor-brand"><img src="scale-gold.png?v=5107" alt="Весы правосудия"><div><b>Ежедневник адвоката</b><small>Больше, чем календарь</small></div></div>'+
...
function quickItem(i,t,act,tone){
  var art={
    'qa-hearing':'quick-icon-hearing.png?v=5107',
    'qa-meeting':'quick-icon-meeting.png?v=5107',
    'qa-deadline':'quick-icon-deadline.png?v=5107',
    'qa-task':'quick-icon-task.png?v=5107',
    'qa-journal':'quick-icon-journal.png?v=5107'
  }[act]||'';
...
function premiumActionIcon(type,cls){
  ...
  var src=type==='bell'?'header-bell-premium.png?v=5107':type==='search'?'header-search-premium.png?v=5107':type==='plus'?'fab-plus-square-premium.png?v=5107':'';
...
function searchPremiumExamples(){
  return '<div class="search-premium-label-row"><span></span><b>ПРИМЕРЫ ЗАПРОСОВ</b><span></span></div>'+
    '<div class="search-premium-chips">'+
      ...
      searchPremiumChip('81 УК','81 УК','doc')+
      searchPremiumChip('Ивановский суд','Ивановский суд','brief')+
      searchPremiumChip('А56-12345/2024','А56-12345/2024','doc')+
      searchPremiumChip('ст. 318 УК','ст. 318 УК','scale')+
...
function sheetGlobalSearch(){
  ...
  openSheet('<div class="search-premium-hero">'+
    ...
    searchPremiumExamples()+
    '<div id="gresults" class="search-premium-results"></div>'+
    searchPremiumHelperCard()+
    '<div class="search-premium-motto"><span></span><i>'+ico('scale','s')+'</i><span></span><b>ПРАВО И ПОРЯДОК</b></div>');
...
function notifyStatusMeta(){
  ...
  if(st==='unsupported') return {title:'Локальные уведомления', text:'Этот браузер не поддерживает уведомления. На iPhone они доступны после установки приложения на экран «Домой».', chip:'Недоступны', tone:'muted'};
  if(st==='denied') return {title:'Локальные уведомления', text:'Уведомления запрещены в настройках iPhone. Разрешите их для приложения, чтобы получать локальные напоминания.', chip:'Запрещены', tone:'warn'};
  if(S.settings.notify) return {title:'Локальные уведомления', text:'Напоминания включены. На iPhone они срабатывают, пока веб-приложение активно; iOS может приостанавливать его в фоне.', chip:'Включены', tone:'ok'};
...
function sheetNotify(){
  ...
  openSheet('<section class="rem107">'+
    '<header class="rem107-head">'+
      '<div class="rem107-head-copy"><h2>Напоминания</h2><p>Локальное напоминание за 10 минут до задачи и за час до заседания.</p></div>'+
      '<img class="rem107-head-bell" src="reminder-approved-top-bell.png?v=5107" alt="">'+
      '<span class="rem107-curve rem107-curve-a" aria-hidden="true"></span>'+
      '<span class="rem107-curve rem107-curve-b" aria-hidden="true"></span>'+
    '</header>'+
    '<article class="rem107-card rem107-card-local">'+
      '<img class="rem107-card-icon" src="reminder-approved-card-bell.png?v=5107" alt="">'+
      '<div class="rem107-card-copy"><h3>'+meta.title+'</h3><p>'+meta.text+'</p>'+
        '<span class="rem107-status '+meta.tone+'"><span>'+ico(statusIco,'s')+'</span>'+meta.chip+'</span>'+
      '</div>'+
      '<span class="rem107-chevron">'+ico('chev','s')+'</span>'+
    '</article>'+
    '<article class="rem107-card rem107-card-calendar">'+
      '<img class="rem107-card-icon" src="reminder-approved-card-calendar.png?v=5107" alt="">'+
      '<div class="rem107-card-copy"><h3>Критичные события —<br>в системный календарь</h3><p>Для судебных заседаний и важных процессуальных сроков дополнительно используйте «Календарь» или «Напоминания» iPhone. Автономное PWA не может гарантировать фоновые таймеры после выгрузки системой.</p></div>'+
      '<span class="rem107-chevron">'+ico('chev','s')+'</span>'+
    '</article>'+
    '<div class="rem107-toggle '+(S.settings.notify?'is-on':'is-off')+'" data-act="notify" role="button" tabindex="0">'+
      '<span class="rem107-toggle-bell">'+ico('bell','s')+'</span>'+
      '<span class="rem107-toggle-text">'+(S.settings.notify?'Выключить напоминания':'Включить напоминания')+'</span>'+
      '<span class="rem107-toggle-chevron">'+ico('chev','s')+'</span>'+
    '</div>'+
    '<p class="rem107-note">Вы всегда сможете включить их снова в настройках.</p>'+
    '<footer class="rem107-footer"><span></span><img src="reminder-approved-footer-scales.png?v=5107" alt=""><span></span><b>ДЕЛА В ПОРЯДКЕ</b></footer>'+
  '</section>');
  $('#sheet').classList.add('reminders-approved-v107-sheet');
}
