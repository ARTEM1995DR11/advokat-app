...
function renderPremiumListPicker(){
  ...
  var headIconHtml=isCourtPicker?'<span class="premium-list-head-icon premium-court-head-scales"><img src="scale-gold.webp?v=5362" alt="Весы правосудия"></span>':'<span class="premium-list-head-icon">'+ico(LIST_PICKER.meta.icon||'list')+'</span>';
  modal.innerHTML='<div class="premium-list-grab"></div>'+ 
    ...
    '<div class="premium-list-sign"><i></i><span><img class="premium-list-sign-logo" src="scale-gold.webp?v=5362" alt="Весы правосудия"></span><i></i></div>';
}
...
function drawHearingResultSheet(){
  ...
  openSheet('<div class="hearing-result-v355-head">'+
    ...
    '<div class="hearing-result-v355-sign" aria-hidden="true"><i></i><span><img src="scale-gold.webp?v=5362" alt="Весы правосудия"></span><i></i></div>');
  var sheet=$('#sheet');
...
function sheetMatterFilters(){
  ...
  openSheet(
    '<div class="matter-filter-premium-head">'+
      '<span class="matter-filter-brand-mark"><img src="scale-gold.webp?v=5362" alt=""></span>'+
...
function renderQuickEntryTop190(title,currentKind){
  ...
  return '<section class="qe190-top qe190-kind-'+esc(currentKind)+(editTitle?' qe190-mode-edit':' qe190-mode-create')+'">'+
    '<div class="qe190-brand"><img src="scale-gold.webp?v=5362" alt="Весы правосудия"><div class="qe190-brand-copy"><b>Ежедневник адвоката</b><small>Больше, чем календарь</small></div></div>'+
...
document.addEventListener('touchmove',function(e){
  ...
  if(!TASK_TOUCH.horizontal){
    // 5.0.362: лёгкий короткий свайп должен подхватываться почти сразу.
    // Вертикальный скролл всё ещё имеет приоритет, если движение явно идёт вниз/вверх.
    if(Math.abs(TASK_TOUCH.dy)>22 && Math.abs(TASK_TOUCH.dy)>Math.abs(TASK_TOUCH.dx)*1.18){taskTouchReset(true);return;}
    if(TASK_TOUCH.dx<-5 && Math.abs(TASK_TOUCH.dx)>Math.abs(TASK_TOUCH.dy)*1.03) TASK_TOUCH.horizontal=true;
  }
  if(!TASK_TOUCH.horizontal) return;
  if(e.cancelable)e.preventDefault();
  var x=Math.max(-92,Math.min(0,TASK_TOUCH.dx));
  TASK_TOUCH.row.style.setProperty('transform','translate3d('+x+'px,0,0)','important');
  TASK_TOUCH.row.classList.toggle('swipe-left',x<=-10);
  TASK_TOUCH.row.classList.toggle('swipe-ready',x<=-32);
},{passive:false,capture:true});
document.addEventListener('touchend',function(){
...
if('serviceWorker' in navigator){
  window.addEventListener('load',function(){
    ...
    setTimeout(function(){
      navigator.serviceWorker.register('./sw.js?v=5362',{updateViaCache:'none'})
        .catch(function(){});
    },1400);
  });
}
