...
function renderTasks(){
  ...
  var html='<div class="tasks-project">'+
    '<div class="today-brand"><div class="today-brand-left"><span class="today-logo"><img src="scale-gold.png?v=323" alt="Весы правосудия"></span><div><b>Ежедневник адвоката</b><small>Больше, чем календарь</small></div></div>'+
...
function taskProjectRow(t){
  ...
  return '<div class="pt-item" data-id="'+t.id+'">'+
    '<div class="pt-swipe-bg"><span></span><span class="pt-swipe-more">Действия'+ico('more','s')+'</span></div>'+ 
    '<div class="pt-row'+(t.done?' done':'')+'" data-id="'+t.id+'">'+
      ...
      '<div class="pt-inline-actions">'+taskActionButtons(t)+'</div></div>';
}
...
/* =====================================================================
   Swipe по задачам — 3.1.25
   Завершение задачи: только кружок слева.
   Свайп справа налево: открыть быстрые действия.
   Левые 44px зарезервированы для системного Back-swipe.
   ===================================================================== */
...
function finishTaskSwipe(){
  if(!TASK_TOUCH.on || !TASK_TOUCH.row) return;
  var dx=TASK_TOUCH.dx,dy=TASK_TOUCH.dy,id=TASK_TOUCH.id,h=TASK_TOUCH.horizontal;
  var trigger=h && id && dx<=-28 && Math.abs(dx)>Math.abs(dy)*1.05;
  var moved=h && Math.abs(dx)>14;
  taskTouchReset(true);
  if(moved) SWIPE_CLICK_BLOCK_UNTIL=Date.now()+420;
  if(trigger){
    setTimeout(function(){
      closeTaskActionRows(id);
      var wrap=document.querySelector('.pt-item[data-id="'+id+'"]');
      if(wrap){wrap.classList.add('open');vib(7);}
    },65);
  }
}
document.addEventListener('touchstart',function(e){
...
document.addEventListener('touchmove',function(e){
  ...
  if(e.cancelable)e.preventDefault();
  // Right swipe is deliberately ignored: the completion circle owns that action.
  var x=Math.max(-88,Math.min(0,TASK_TOUCH.dx));
  TASK_TOUCH.row.style.transform='translate3d('+x+'px,0,0)';
  TASK_TOUCH.row.classList.toggle('swipe-left',x<-10);
  TASK_TOUCH.row.classList.toggle('swipe-ready',x<=-28);
},{passive:false,capture:true});
document.addEventListener('touchend',finishTaskSwipe,{passive:true,capture:true});
document.addEventListener('touchcancel',finishTaskSwipe,{passive:true,capture:true});
...
if('serviceWorker' in navigator){
  window.addEventListener('load',function(){
    ...
    navigator.serviceWorker.register('./sw-3125.js', {updateViaCache:'none'}).then(function(reg){
