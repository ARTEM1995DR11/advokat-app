...
var DEF = {
  ...
  ui: {
    ...
    showArch:false, matterType:'', matterStage:'', taskGroupOpen:{}
  }
};
...
function taskCard(t,opts){
  ...
  return '<div class="task p-'+t.pri+' k-'+t.kind+(t.done?' done':'')+'" data-act="task" data-id="'+t.id+'">'+
    (hearing?'<div class="eventmark">'+ico('gavel')+'</div>':'<button class="chk" data-act="toggle" data-id="'+t.id+'">'+ico('check')+'</button>')+
    '<div class="tbody">'+
      '<div class="trow">'+
...
function todayDeadlineRow(t){
  ...
  return '<button class="today-row deadline-row" data-act="task" data-id="'+t.id+'">'+
    ...
    '<small>'+esc(m?(m.title+(m.number?' · '+m.number:'')):'Без привязки к делу')+'</small>'+
    '<em>Просрочен'+(late<-1?' на '+(-late)+' дн.':'')+'</em></span>'+ico('chev','s')+'</button>';
}
function todayHearingRow(t){
  var m=todayMatter(t), place=t.place||(m&&m.court)||'';
  return '<button class="today-row hearing-row" data-act="task" data-id="'+t.id+'">'+
    ...
    '<small>'+esc(place||'Суд не указан')+'</small>'+(t.note?'<small>'+esc(t.note)+'</small>':'')+'</span>'+ico('chev','s')+'</button>';
}
function todayTaskRow(t){
  ...
  return '<div class="today-row task-row" data-act="task" data-id="'+t.id+'">'+
    ...
    '<small>'+esc(m?(m.title+(m.number?' · '+m.number:'')):(t.note||'Без дела'))+'</small></span>'+
    (t.time?'<span class="today-row-time mono">'+esc(t.time)+'</span>':'')+'</div>';
}
function todayUpcomingRow(t){
  var m=todayMatter(t), d=parseD(t.due), place=t.place||(m&&m.court)||'';
...
function promptTaskDate(cur){
  var v=prompt('Новая дата в формате ГГГГ-ММ-ДД', cur||today());
  if(v===null) return null;
  v=v.trim();
  if(!/^\d{4}-\d{2}-\d{2}$/.test(v)){ toast('Дата должна быть в формате ГГГГ-ММ-ДД'); return false; }
  return v;
}
...
function sheetTaskActions(id){
  ...
  if(!t) return;
  var m=t.mid?matter(t.mid):null;
  var sub=(m?([m.number,m.title].filter(Boolean).join(' · ')):'Без дела');
  var rows='';
  if(t.done){
    rows+='<button class="row" data-act="task-action-toggle" data-id="'+t.id+'">'+ico('check')+'<span class="rl">Вернуть в работу<small>Снять отметку о выполнении</small></span>'+ico('chev','s')+'</button>';
  }else{
    if(t.due && dd(t.due)<0){
      rows+='<button class="row" data-act="task-action-today" data-id="'+t.id+'">'+ico('sun')+'<span class="rl">Перенести на сегодня<small>'+fmtD(today(),true)+'</small></span>'+ico('chev','s')+'</button>'+
            '<button class="row" data-act="task-action-tomorrow" data-id="'+t.id+'">'+ico('cal')+'<span class="rl">Перенести на завтра<small>'+fmtD(addD(today(),1),true)+'</small></span>'+ico('chev','s')+'</button>';
    }
    rows+='<button class="row" data-act="task-action-date" data-id="'+t.id+'">'+ico('cal')+'<span class="rl">Выбрать дату<small>'+(t.due?('Сейчас: '+fmtD(t.due,true)):'Срок не установлен')+'</small></span>'+ico('chev','s')+'</button>';
  }
  rows+='<button class="row" data-act="task-action-edit" data-id="'+t.id+'">'+ico('edit')+'<span class="rl">Изменить задачу<small>Название, срок, приоритет и дело</small></span>'+ico('chev','s')+'</button>'+
        '<button class="row danger-row" data-act="task-action-delete" data-id="'+t.id+'">'+ico('trash')+'<span class="rl">Удалить<small>Удалить задачу без возможности восстановления</small></span>'+ico('chev','s')+'</button>';
  openSheet('<h2>Действия с задачей</h2><p class="sh-sub">'+esc(t.title)+' · '+esc(sub)+'</p><div class="card pad0 task-action-sheet">'+rows+'</div>');
}
...
function renderTasks(){
  ...
  var html='<div class="tasks-project">'+
    '<div class="today-brand"><div class="today-brand-left"><span class="today-logo"><img src="scale-gold.png?v=326" alt="Весы правосудия"></span><div><b>Ежедневник адвоката</b><small>Больше, чем календарь</small></div></div>'+
...
function taskProjectGroups(list){
  var out=[
    ...
    {key:'pasthear',title:'Прошедшие заседания',tone:'slate',items:[]},
...
function taskActionButtons(t){
  var acts=[];
  if(!t.done){
    acts.push('<button class="pt-act soft" data-act="task-move-today" data-id="'+t.id+'">Сегодня</button>');
    acts.push('<button class="pt-act soft" data-act="task-move-tomorrow" data-id="'+t.id+'">Завтра</button>');
  } else {
    acts.push('<button class="pt-act soft" data-act="toggle" data-id="'+t.id+'">В работу</button>');
  }
  acts.push('<button class="pt-act" data-act="task" data-id="'+t.id+'">Изменить</button>');
  acts.push('<button class="pt-act danger" data-act="task-del" data-id="'+t.id+'">Удалить</button>');
  return acts.join('');
}
function taskProjectRow(t){
  ...
  return '<div class="pt-item" data-id="'+t.id+'">'+
    ...
    '<div class="pt-row'+(t.done?' done':'')+'" data-id="'+t.id+'">'+
      ...
      '<div class="pt-main">'+
        ...
        (m?'<button class="pt-link" data-act="task-matter" data-id="'+m.id+'">'+esc(context)+'</button>':'<span>'+esc(context)+'</span>')+
        due+
      '</div>'+
...
function renderMore(){
  ...
  '</div>'+
  '<div class="footnote">Ежедневник адвоката · iPhone Offline 3.1.19<br>'+esc(offlineStatusText())+'<br>Рабочая база хранится локально в зашифрованном виде.</div>';
...
function drawEditor(){
  ...
  (isNew?'':((hearing||t.kind==='deadline'||t.kind==='meeting')?'<button class="btn ghost" data-act="ics-task" data-id="'+t.id+'" style="margin-top:8px">Добавить в календарь iPhone</button>':'')+
...
document.addEventListener('click', function(ev){
  ...
  switch(a){
    ...
    case 'task-action-toggle': closeSheet(); toggleTaskDone(id); break;
    case 'task-action-today': closeSheet(); moveTaskToDate(id,today(),'Перенесено на сегодня'); break;
    case 'task-action-tomorrow': closeSheet(); moveTaskToDate(id,addD(today(),1),'Перенесено на завтра'); break;
    case 'task-action-date': {closeSheet();var at=S.tasks.filter(function(x){return x.id===id;})[0];if(!at)break;var ad=promptTaskDate(at.due||today());if(ad===false||ad===null)break;moveTaskToDate(id,ad,'Новая дата');break;}
    case 'task-action-edit': {closeSheet();var ae=S.tasks.filter(function(x){return x.id===id;})[0];if(ae)editTask(ae);break;}
    case 'task-action-delete': closeSheet(); deleteTaskById(id); break;
    case 'task-del': deleteTaskById(id); break;
    case 'task-move-today': moveTaskToDate(id,today(),'Перенесено на сегодня'); break;
    case 'task-move-tomorrow': moveTaskToDate(id,addD(today(),1),'Перенесено на завтра'); break;
    case 'task-move-date': {var tk=S.tasks.filter(function(x){return x.id===id;})[0]; if(!tk) break; var nd=promptTaskDate(tk.due||today()); if(nd===false||nd===null) break; moveTaskToDate(id,nd,'Новая дата'); break;}
...
/* =====================================================================
   Swipe по задачам — 3.1.26
   Завершение: кружок слева.
   Свайп справа налево: сразу открыть шторку действий при достижении порога.
   ===================================================================== */
var TASK_TOUCH={on:false,row:null,id:'',sx:0,sy:0,dx:0,dy:0,horizontal:false,fired:false};
function taskTouchReset(animate){
  ...
  if(row){
    ...
    if(animate) setTimeout(function(){row.classList.remove('swipe-snap');},180);
  }
  TASK_TOUCH.on=false;TASK_TOUCH.row=null;TASK_TOUCH.id='';TASK_TOUCH.dx=0;TASK_TOUCH.dy=0;TASK_TOUCH.horizontal=false;
}
function fireTaskSwipeActions(){
  if(!TASK_TOUCH.on || TASK_TOUCH.fired || !TASK_TOUCH.id) return;
  var id=TASK_TOUCH.id;
  TASK_TOUCH.fired=true;
  SWIPE_CLICK_BLOCK_UNTIL=Date.now()+650;
  taskTouchReset(false);
  vib(7);
  setTimeout(function(){sheetTaskActions(id);},20);
}
document.addEventListener('touchstart',function(e){
  ...
  TASK_TOUCH.sx=t.clientX;TASK_TOUCH.sy=t.clientY;TASK_TOUCH.dx=0;TASK_TOUCH.dy=0;TASK_TOUCH.horizontal=false;TASK_TOUCH.fired=false;
...
document.addEventListener('touchmove',function(e){
  ...
  if(!TASK_TOUCH.horizontal){
    if(Math.abs(TASK_TOUCH.dy)>12 && Math.abs(TASK_TOUCH.dy)>Math.abs(TASK_TOUCH.dx)*1.2){taskTouchReset(true);return;}
    if(Math.abs(TASK_TOUCH.dx)>7 && Math.abs(TASK_TOUCH.dx)>Math.abs(TASK_TOUCH.dy)*1.05) TASK_TOUCH.horizontal=true;
  }
  if(!TASK_TOUCH.horizontal) return;
  if(e.cancelable)e.preventDefault();
  var x=Math.max(-72,Math.min(0,TASK_TOUCH.dx));
  TASK_TOUCH.row.style.transform='translate3d('+x+'px,0,0)';
  TASK_TOUCH.row.classList.toggle('swipe-left',x<-8);
  TASK_TOUCH.row.classList.toggle('swipe-ready',x<=-40);
  if(x<=-48) fireTaskSwipeActions();
},{passive:false,capture:true});
document.addEventListener('touchend',function(){
...
document.addEventListener('touchcancel',function(){
  if(!TASK_TOUCH.on) return;
  if(TASK_TOUCH.horizontal && TASK_TOUCH.dx<=-40) fireTaskSwipeActions();
  else taskTouchReset(true);
},{passive:true,capture:true});
...
if('serviceWorker' in navigator){
  window.addEventListener('load',function(){
    ...
    navigator.serviceWorker.register('./sw-3126.js', {updateViaCache:'none'}).then(function(reg){
