...
function bindTaskRowGestures(){
  var rows=$$('#tasklist .pt-row');
  rows.forEach(function(row){
    if(row.dataset.swipeBound==='1') return;
    row.dataset.swipeBound='1';
    var st={on:false,id:null,sx:0,sy:0,dx:0,dy:0,moved:false};
    row.addEventListener('pointerdown',function(e){
      if(e.pointerType==='mouse') return;
      if(e.clientX<=44) return; // reserve left edge for app Back gesture
      st.on=true;st.id=e.pointerId;st.sx=e.clientX;st.sy=e.clientY;st.dx=0;st.dy=0;st.moved=false;
    },{passive:true});
    row.addEventListener('pointermove',function(e){
      if(!st.on || e.pointerId!==st.id) return;
      st.dx=e.clientX-st.sx; st.dy=e.clientY-st.sy;
      if(Math.abs(st.dx)>8 && Math.abs(st.dx)>Math.abs(st.dy)*1.1){
        st.moved=true;
        try{row.setPointerCapture(e.pointerId);}catch(_){ }
        var x=Math.max(-76,Math.min(76,st.dx));
        row.style.transform='translateX('+x+'px)';
        row.classList.toggle('swipe-right',x>12);
        row.classList.toggle('swipe-left',x<-12);
        if(e.cancelable)e.preventDefault();
      }
    },{passive:false});
    function finish(e){
      if(!st.on || (e.pointerId!=null && e.pointerId!==st.id)) return;
      var dx=st.dx,dy=st.dy,moved=st.moved,id=row.dataset.id;
      st.on=false;
      row.style.transform='';
      row.classList.remove('swipe-right','swipe-left');
      if(!moved || Math.abs(dx)<48 || Math.abs(dx)<Math.abs(dy)*1.15) return;
      SWIPE_CLICK_BLOCK_UNTIL=Date.now()+420;
      if(dx>0){ closeTaskActionRows(); toggleTaskDone(id); }
      else { toggleTaskActionRow(id); }
    }
    row.addEventListener('pointerup',finish,{passive:true});
    row.addEventListener('pointercancel',finish,{passive:true});
  });
}
...
if('serviceWorker' in navigator){
  window.addEventListener('load',function(){
    ...
    navigator.serviceWorker.register('./sw-3123.js', {updateViaCache:'none'}).then(function(reg){
