...
function renderTasks(){
  ...
  var html='<div class="tasks-project">'+
    '<div class="tasks-brand"><div class="today-brand-left"><span class="today-logo"><img src="scale-gold.png?v=321" alt="Весы правосудия"></span><div><b>Ежедневник адвоката</b><small>Больше, чем календарь</small></div></div>'+
      '<div class="tasks-project-actions"><button class="iconbtn'+(u.q?' on':'')+'" data-act="search" title="Поиск">'+ico('search')+'</button></div></div>'+
    '<div class="tasks-project-head"><div><p>Всего '+c.all+' в работе</p><h1>Задачи</h1></div></div>'+
...
var TASK_ROW_SWIPE={on:false,x:0,y:0,dx:0,dy:0,id:'',node:null};
document.addEventListener('touchstart',function(e){
  if(!unlocked || !e.touches || e.touches.length!==1) return;
  if(e.target.closest('#sheet.open')) return;
  if(e.target.closest('[data-act="toggle"]')) return;
  var row=e.target.closest('.pt-row');
  if(!row) return;
  var t=e.touches[0];
  TASK_ROW_SWIPE.on=true; TASK_ROW_SWIPE.x=t.clientX; TASK_ROW_SWIPE.y=t.clientY;
  TASK_ROW_SWIPE.dx=0; TASK_ROW_SWIPE.dy=0; TASK_ROW_SWIPE.id=row.dataset.id||''; TASK_ROW_SWIPE.node=row;
},{passive:true,capture:true});
document.addEventListener('touchmove',function(e){
  if(!TASK_ROW_SWIPE.on || !e.touches || e.touches.length!==1) return;
  var t=e.touches[0];
  TASK_ROW_SWIPE.dx=t.clientX-TASK_ROW_SWIPE.x; TASK_ROW_SWIPE.dy=t.clientY-TASK_ROW_SWIPE.y;
  if(Math.abs(TASK_ROW_SWIPE.dx)>12 && Math.abs(TASK_ROW_SWIPE.dx)>Math.abs(TASK_ROW_SWIPE.dy)*1.2){
    if(e.cancelable) e.preventDefault();
  }
},{passive:false,capture:true});
document.addEventListener('touchend',function(){
  if(!TASK_ROW_SWIPE.on) return;
  var dx=TASK_ROW_SWIPE.dx, dy=TASK_ROW_SWIPE.dy, id=TASK_ROW_SWIPE.id;
  TASK_ROW_SWIPE.on=false;
  if(!id) return;
  if(Math.abs(dx)<54 || Math.abs(dx)<Math.abs(dy)*1.2) return;
  SWIPE_CLICK_BLOCK_UNTIL=Date.now()+360;
  if(dx>0){
    closeTaskActionRows();
    toggleTaskDone(id);
  } else {
    toggleTaskActionRow(id);
  }
},{passive:true,capture:true});
document.addEventListener('touchcancel',function(){TASK_ROW_SWIPE.on=false;},{passive:true,capture:true});
...
if('serviceWorker' in navigator){
  window.addEventListener('load',function(){
    ...
    navigator.serviceWorker.register('./sw-3122.js', {updateViaCache:'none'}).then(function(reg){
