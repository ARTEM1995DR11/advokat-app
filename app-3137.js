...
function drawEditor(){
  ...
  var kindIcons={task:'check',hearing:'cal',deadline:'clock'};
  ...
  openSheet(
  '<div class="task-editor-brand"><img src="scale-gold.png?v=3137" alt="Весы правосудия"><div><b>Ежедневник адвоката</b><small>Больше, чем календарь</small></div></div>'+
  ...
  (!hearing?'<div class="fld task-editor-title-field"><label>Что нужно сделать</label><input id="e-title" placeholder="Подготовить апелляционную жалобу" value="'+esc(t.title)+'" autocomplete="off"></div>':'')+
  ...
  (!hearing?'<div class="fld task-priority-field"><label>Приоритет</label><div class="chips task-priority-chips">'+pris+'</div></div>':'')+
  (hearing
  ...
  '<div class="fld task-editor-note-field"><label>'+(hearing?'Примечание (необязательно)':'Примечание')+'</label>'+
  ...
  '<button class="btn task-editor-save" data-act="e-save"><span class="save-icon">'+ico('save','s')+'</span>'+(hearing?'Сохранить заседание':'Сохранить')+'</button>'+
...
if('serviceWorker' in navigator){
  window.addEventListener('load',function(){
    ...
    navigator.serviceWorker.register('./sw-3137.js', {updateViaCache:'none'}).then(function(reg){
