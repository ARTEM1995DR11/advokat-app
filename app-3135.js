...
function mergeState(d){
  ...
  out.tasks.forEach(function(t){
    if(!t) return;
    if(t.kind==='meeting' || t.kind==='call' || t.kind==='doc') t.kind='task';
...
async function loadForSecret(secret){
  ...
  if(payload){
    ...
    if(cleanupOldCompletedTasks(30)) await persistNow();
    return S;
  }
...
var EDITOR_KINDS = ['task','hearing','deadline'];
...
function todayHearingRow(t){
  var m=todayMatter(t), place=t.place||(m&&m.court)||'', sub=hearingSubcaption(t,m);
  return '<button class="today-row hearing-row" data-act="task" data-id="'+t.id+'">'+
    ...
    (sub?'<small>'+esc(sub)+'</small>':'')+'<small>'+esc(place||'Суд не указан')+'</small>'+(t.note?'<small>'+esc(t.note)+'</small>':'')+'</span>'+ico('chev','s')+'</button>';
}
...
function todayUpcomingRow(t){
  var m=todayMatter(t), d=parseD(t.due), place=t.place||(m&&m.court)||'', sub=hearingSubcaption(t,m);
  return '<button class="today-row upcoming-row" data-act="task" data-id="'+t.id+'">'+
    ...
    '<span class="today-time mono">'+esc(t.time||'—:—')+'</span><span class="today-row-main"><b>'+esc(hearingCaption(t,m))+'</b>'+
    (sub?'<small>'+esc(sub)+'</small>':'')+'<small>'+esc(place||'Суд не указан')+'</small></span>'+ico('chev','s')+'</button>';
}
...
function taskProjectBase(){
  ...
  return S.tasks.filter(function(t){
    if(!q) return true;
    var m=t.mid?matter(t.mid):null;
    var hay=(t.title+' '+(t.note||'')+' '+(t.hearingClient||'')+' '+(t.hearingNumber||'')+' '+(m?m.title+' '+(m.client||'')+' '+(m.number||''):'')).toLowerCase();
...
function matterCard(m){
  ...
  return '<article class="mp-card'+(m.archived?' archived':'')+'" data-act="matter" data-id="'+m.id+'">'+
    '<div class="mp-top"><span class="mp-case-icon" style="--case:'+mt.c+'">'+ico('brief')+'</span><div class="mp-heading"><b>'+esc(primary)+'</b><small>'+esc(mt.n)+'</small></div>'+
...
function sheetMatterFilters(){
  var rows='<button class="row" data-act="m-filter" data-v="">'+ico('folder')+'<span class="rl">Все производства<small>Показывать дела всех типов</small></span>'+(S.ui.matterType===''?ico('check','s'):ico('chev','s'))+'</button>'+
  ...
  openSheet('<h2>Фильтр дел</h2><p class="sh-sub">Тип производства</p><div class="card pad0 matter-filter-sheet">'+rows+'</div>');
}
function renderMatters(){
  ...
  var html='<div class="matters-project">'+
    ...
    '<div class="matters-scope">'+
      ...
      '<button class="matter-filter-btn'+(S.ui.matterType?' on':'')+'" data-act="matter-filter-sheet" title="Фильтр по типу">'+ico('list','s')+'<span>'+typeName+'</span></button></div>'+
...
function editTask(t,preset){
  ED = t ? JSON.parse(JSON.stringify(t))
    : Object.assign({ id:null,title:'',mid:'',note:'',due:'',time:'',place:'',pri:'mid',kind:'task',sourceDate:'',rule:'',hearingClient:'',hearingNumber:'',
...
function drawEditor(){
  ...
  var title = hearing ? (isNew?'Новое заседание':'Редактирование заседания') : (isNew?'Новая задача':'Редактирование задачи');

  openSheet(
  '<div class="shhead task-editor-head"><h2>'+title+'</h2></div>'+
  ...
  '<div class="fld"><label>Тип</label><div class="chips task-kind-chips">'+kinds+'</div></div>'+
  ...
  (hearing
    ? '<div class="fld"><label>Суд / место заседания *</label><input id="e-place" placeholder="Например: Кинешемский городской суд, зал 3" value="'+esc(t.place||'')+'"><small class="fieldhint">Если выбрано постоянное дело, суд подставится автоматически. Для разового заседания укажите суд вручную.</small></div>'
    : '')+
  ...
  '<div class="fld"><label>'+(hearing?'Примечание (необязательно)':'Примечание')+'</label>'+
    '<textarea id="e-note" class="task-note-editor" rows="5" placeholder="'+(hearing?'Например: зал 3, взять оригиналы документов':'Нормы права, документы, что взять с собой…')+'">'+esc(t.note||'')+'</textarea></div>'+
...
function globalSearchData(q){
  ...
  var ts=S.tasks.filter(function(t){var m=t.mid?matter(t.mid):null;return [t.title,t.note,t.place,t.rule,t.hearingClient,t.hearingNumber,m&&m.title,m&&m.client,m&&m.number].some(has);}).sort(sortT).slice(0,12);
...
function sheetBackup(){
  ...
  openSheet('<h2>Резервная копия планов</h2><p class="sh-sub">В копию войдут все дела, задачи, заседания и процессуальные сроки. Файл можно сохранить в «Файлы» / iCloud Drive и потом восстановить приложение.</p>'+
    ...
    '<div class="hint">Пароль нужен для восстановления на этом или другом iPhone. Он не сохраняется в приложении.</div><button class="btn" data-act="backup-create">Сохранить резервную копию</button>');
}
async function createBackupFile(){
  ...
  try{
    ...
    S.settings.lastBackup=new Date().toISOString();save();closeSheet();render();toast('Резервная копия создана');
...
if('serviceWorker' in navigator){
  window.addEventListener('load',function(){
    ...
    navigator.serviceWorker.register('./sw-3135.js', {updateViaCache:'none'}).then(function(reg){
